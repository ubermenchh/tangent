...
export function initializeSkills(): void {
    ...
    skillRegistry.register(whatsappSkill, [
        "whatsapp", "whats app",
        "message", "chat", "dm", "direct message",
        "send message", "read message", "check message",
    ]);

    skillRegistry.register(twitterSkill, [
        "twitter", "x", "tweet", "tweets",
        "timeline", "feed", "retweet", "like",
        "post", "thread",
    ]);
    ...
    skillRegistry.register(facebookSkill, [
        "facebook", "fb",
        "news feed",
    ]);
    ...
    skillRegistry.register(amazonSkill, [
        ...
        "review", "reviews", "deal", "deals",
    ]);

    skillRegistry.register(zomatoSkill, [
        "zomato",
        "food", "restaurant", "order food",
        "delivery", "menu", "cuisine",
    ...
    skillRegistry.register(blinkitSkill, [
        "blinkit", "blink it",
        "grocery", "groceries",
        "instant delivery", "quick delivery",
        "vegetables", "fruits", "dairy", "snacks",
    ]);

    skillRegistry.register(productivitySkill, [
        "reminder", "remind", "calendar", "event", "meeting",
        "schedule", "email", "gmail", "note", "notes", "keep",
        "todo", "task", "deadline",
    ]);

    skillRegistry.register(navigationSkill, [
        "navigate", "directions", "map", "maps", "route",
        "uber", "ola", "cab", "taxi", "drive", "walk",
        "transit", "bus", "train", "commute", "traffic",
    ]);

    skillRegistry.register(entertainmentSkill, [
        "play", "music", "song", "video", "youtube", "spotify",
        "netflix", "watch", "listen", "stream", "movie",
        "show", "podcast", "album", "playlist",
    ]);
